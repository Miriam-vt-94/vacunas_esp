...
idx <- lapply(tabla_sep_lineas,
              ...
              "ANDALUCÍA")
idx_final <- 
  lapply(tabla_sep_lineas,
         FUN = function(x, marca) { which(grepl(marca, x)) },
         "TOTALES")
idx_final <- mapply(idx_final, FUN = function(x, idx) { x[x > idx] }, idx)

# Tabla de tamaño n_ccaa x n_fechas
tabla_valores <- mapply(tabla_sep_lineas,
                        FUN = function (x, idx, idx_final) { x[idx:idx_final] },
                        idx, idx_final)
...
for (i in 1:length(idx_fechas)) {
  ...
  datos_depurados <- 
    sapply(datos_split,
           FUN = function(x) { as.numeric(x[x != "" &
                                              !is.na(as.numeric(x))]) })
  datos_depurados <- datos_depurados[datos_depurados != "numeric(0)"]
  datos_depurados <-
    matrix(unlist(datos_depurados), ncol = length(ccaa$NOMBRES) + 1)
  datos_depurados <- datos_depurados[, -dim(datos_depurados)[2]]
...
# #######################################################
# SCRAPPING PDF VACUNAS: DESDE 24 MARZO HASTA 5 DE ABRIL INCLUIDOS
# #######################################################
...
# #######################################################
# SCRAPPING PDF VACUNAS: DESDE 6 DE ABRIL INCLUIDOS
# #######################################################
...
pdf_filtrado <- list()
idx_fechas <- which(names(pdf_bruto) %in%
                      as.character(seq(as.Date("2021-04-06"),
...
for (i in 1:length(idx_fechas)) {
  ...
  datos_depurados <- 
    sapply(datos_split,
           FUN = function(x) { as.numeric(x[x != "" &
                                              !is.na(as.numeric(x))]) })
  datos_depurados <- datos_depurados[-7, ]
  datos_depurados <-
    matrix(unlist(datos_depurados), ncol = length(ccaa$NOMBRES) + 1)
...
aux <- panel_vacunas_fecha
seq_fechas <- seq(min(as.Date(names(panel_vacunas_fecha))),
                  max(as.Date(names(panel_vacunas_fecha))), by = 1)
panel_vacunas_fecha <- list()
for (i in 1:length(seq_fechas)) {

  if (any(names(aux) == seq_fechas[i])) {
    
    panel_vacunas_fecha[[i]] <-
      aux[[which(names(aux) == seq_fechas[i])]]
    
  } else {
    
    panel_vacunas_fecha[[i]] <- panel_vacunas_fecha[[i - 1]]
      
  }
}
names(panel_vacunas_fecha) <- seq_fechas
...
# Para calcular lo diario restamos el acumulado de un día menos otro
panel_vacunas_fecha[[1]]$DOSIS_DIARIAS_ENTREGADAS <-
  panel_vacunas_fecha[[1]]$DOSIS_ENTREGADAS
panel_vacunas_fecha[[1]]$DOSIS_DIARIAS_ENTREGADAS_PFIZER <-
  panel_vacunas_fecha[[1]]$DOSIS_ENTREGADAS_PFIZER
panel_vacunas_fecha[[1]]$DOSIS_DIARIAS_ENTREGADAS_MODERNA <-
  panel_vacunas_fecha[[1]]$DOSIS_ENTREGADAS_MODERNA
panel_vacunas_fecha[[1]]$DOSIS_DIARIAS_ENTREGADAS_ASTRA_ZENECA <-
  panel_vacunas_fecha[[1]]$DOSIS_ENTREGADAS_ASTRA_ZENECA
panel_vacunas_fecha[[1]]$PAUTA_COMPLETA_DIARIA <-
  panel_vacunas_fecha[[1]]$PAUTA_COMPLETA
panel_vacunas_fecha[[1]]$DOSIS_ADMIN_DIARIAS <-
  panel_vacunas_fecha[[1]]$DOSIS_ADMIN
panel_vacunas_fecha[[1]]$PORC_PAUTA_COMPLETA_DIARIA <-
  panel_vacunas_fecha[[1]]$PORC_PAUTA_COMPLETA
panel_vacunas_fecha[[1]]$DOSIS_ADMIN_100HAB_DIARIAS <-
  panel_vacunas_fecha[[1]]$DOSIS_ADMIN_100HAB
for (i in 2:length(panel_vacunas_fecha)) {
  
  panel_vacunas_fecha[[i]]$DOSIS_DIARIAS_ENTREGADAS <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS - 
    panel_vacunas_fecha[[i - 1]]$DOSIS_ENTREGADAS
  panel_vacunas_fecha[[i]]$DOSIS_DIARIAS_ENTREGADAS_PFIZER <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_PFIZER - 
    panel_vacunas_fecha[[i - 1]]$DOSIS_ENTREGADAS_PFIZER
  panel_vacunas_fecha[[i]]$DOSIS_DIARIAS_ENTREGADAS_MODERNA <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_MODERNA - 
    panel_vacunas_fecha[[i - 1]]$DOSIS_ENTREGADAS_MODERNA
  panel_vacunas_fecha[[i]]$DOSIS_DIARIAS_ENTREGADAS_ASTRA_ZENECA <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_ASTRA_ZENECA -
    panel_vacunas_fecha[[i - 1]]$DOSIS_ENTREGADAS_ASTRA_ZENECA
  panel_vacunas_fecha[[i]]$PAUTA_COMPLETA_DIARIA <-
    panel_vacunas_fecha[[i]]$PAUTA_COMPLETA - 
    panel_vacunas_fecha[[i - 1]]$PAUTA_COMPLETA
  panel_vacunas_fecha[[i]]$DOSIS_ADMIN_DIARIAS <-
    panel_vacunas_fecha[[i]]$DOSIS_ADMIN - 
    panel_vacunas_fecha[[i - 1]]$DOSIS_ADMIN
  
  panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA_DIARIA <-
    round(100 * panel_vacunas_fecha[[i]]$PAUTA_COMPLETA_DIARIA /
            sum(poblacion$poblacion), 3)
  panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB_DIARIAS <-
    round(100 * panel_vacunas_fecha[[1]]$DOSIS_ADMIN_DIARIAS /
            sum(poblacion$poblacion), 3)
}

# Para calcular lo diario restamos el acumulado de un día menos hace 7
for (i in 1:7) {
  
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS_PFIZER <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_PFIZER
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS_MODERNA <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_MODERNA
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS_ASTRA_ZENECA <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_ASTRA_ZENECA
  panel_vacunas_fecha[[i]]$PAUTA_COMPLETA_7D <-
    panel_vacunas_fecha[[i]]$PAUTA_COMPLETA
  panel_vacunas_fecha[[i]]$DOSIS_ADMIN_7D <-
    panel_vacunas_fecha[[i]]$DOSIS_ADMIN
  panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA_7D <-
    panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA
  panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB_7D <-
    panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB
  panel_vacunas_fecha[[i]]$DESV_DOSIS_ADMIN_7D <- 
    panel_vacunas_fecha[[i]]$DESV_DOSIS_ADMIN
  panel_vacunas_fecha[[i]]$DESV_PORC_PAUTA_COMPLETA_7D <- 
    panel_vacunas_fecha[[i]]$DESV_PORC_PAUTA_COMPLETA
  
}

for (i in 8:length(panel_vacunas_fecha)) {
  
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS - 
    panel_vacunas_fecha[[i - 7]]$DOSIS_ENTREGADAS
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS_PFIZER <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_PFIZER - 
    panel_vacunas_fecha[[i - 7]]$DOSIS_ENTREGADAS_PFIZER
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS_MODERNA <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_MODERNA - 
    panel_vacunas_fecha[[i - 7]]$DOSIS_ENTREGADAS_MODERNA
  panel_vacunas_fecha[[i]]$DOSIS_7D_ENTREGADAS_ASTRA_ZENECA <-
    panel_vacunas_fecha[[i]]$DOSIS_ENTREGADAS_ASTRA_ZENECA -
    panel_vacunas_fecha[[i - 7]]$DOSIS_ENTREGADAS_ASTRA_ZENECA
  panel_vacunas_fecha[[i]]$PAUTA_COMPLETA_7D <-
    panel_vacunas_fecha[[i]]$PAUTA_COMPLETA - 
    panel_vacunas_fecha[[i - 7]]$PAUTA_COMPLETA
  panel_vacunas_fecha[[i]]$DOSIS_ADMIN_7D <-
    panel_vacunas_fecha[[i]]$DOSIS_ADMIN - 
    panel_vacunas_fecha[[i - 7]]$DOSIS_ADMIN
  
  panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA_7D <-
    round(100 * panel_vacunas_fecha[[i]]$PAUTA_COMPLETA_7D /
            sum(poblacion$poblacion), 3)
  panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB_7D <-
    round(100 * panel_vacunas_fecha[[i]]$DOSIS_ADMIN_7D /
            sum(poblacion$poblacion), 3)
  
  panel_vacunas_fecha[[i]]$DESV_DOSIS_ADMIN_7D <- 
    100 * (panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB_7D /
             panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB_7D[
               length(panel_vacunas_fecha[[i]]$DOSIS_ADMIN_100HAB_7D)] - 1)
  panel_vacunas_fecha[[i]]$DESV_PORC_PAUTA_COMPLETA_7D <- 
    100 * (panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA_7D /
             panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA_7D[
               length(panel_vacunas_fecha[[i]]$PORC_PAUTA_COMPLETA_7D)] - 1)
}
