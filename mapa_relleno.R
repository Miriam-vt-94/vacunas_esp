...
datos_esp <- datos_esp[dim(datos_esp)[1], ]
...
p <- ggplot(esp_tiles) +
  ...
  theme(text = element_text(family = "Arial"),
    ...
    plot.subtitle =
      element_markdown(size = 13, color = "gray25", lineheight = 1.2),
    plot.caption =
      element_text(color = "gray50", hjust = 0, size = 11),
  ...
  geom_text(
    ...
    size = 4.5, color = "gray60", family = "Arial") +
  geom_text(
    data = pct_labels[pct_labels$point_to_label == "10%", ],
    aes(x = start_line - 5000, y = y, label = "10% (3 mayo)"),
    size = 5.5, color = "#129E75", family = "Arial") +
  geom_text(
    data = pct_labels[pct_labels$point_to_label == "70%", ],
    aes(x = start_line - 10000, y = y, label = point_to_label),
    size = 6, color = "#D92828", family = "Arial") +
...
# GUARDAMOS
# ######################
ggsave(paste0("./GRAFICAS/mapa_vacunacion_esp_",
              as.character(datos_esp$fechas), ".png"),
       p, height = 10, width = 15, device = ragg::agg_png())
